...
BOOL ModalSaveWarning::OnInitDialog() {
	...
	switch(alertType) {
	case MODIFIED_SND:
		GET_CTRL(CStatic, IDC_STATIC_INFO)->SetWindowText(L"当前正在编辑的音效，\r\n还没有被确认修改喵。\r\n\r\n是否确认您的修改记录喵？");
	...
	case MODIFIED_NPK:
		...
		GET_CTRL(CButton, IDC_BUTTON_SAVELEFT)->ShowWindow(SW_HIDE);
		break;
	case MODIFIED_SND_NPK:
		GET_CTRL(CStatic, IDC_STATIC_INFO)->SetWindowText(L"当前打开的NPK文件中，\r\n还有正在被编辑的音效。\r\n\r\n是否保存您的修改记录喵？");
		GET_CTRL(CButton, IDC_BUTTON_CANCEL)->SetWindowText(L"取消");
		GET_CTRL(CButton, IDC_BUTTON_CONTINUE)->SetWindowText(L"不保存NPK");
		GET_CTRL(CButton, IDC_BUTTON_SAVERIGHT)->SetWindowText(L"不修改音效并保存NPK");
		GET_CTRL(CButton, IDC_BUTTON_SAVELEFT)->SetWindowText(L"修改音效并保存NPK");
		break;
	}
