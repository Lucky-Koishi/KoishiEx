...
///////////////////////////////////////////////////////////////////////////////////////
	namespace GIF{
		...
		private:
			void init(uchar LWZminCodeSize = 0);		//0为不重新设置，一般设置为GIF内部数据头的LZWminCode的大小（普遍为8）
			void add(word parentID, uchar content);
			word find(const stream &s);
			bool get(word ID, stream &s);
		private:
			static int LZWgetBitLen(uchar cMinCodeSize, word cNodeCount);	//根据当前节点的数目和最小LZW编码位数获得当前LZW编码位数 
			static void LZWwriteWord(stream &res, longex &startBitPos, int bitCount, word w);	//LZW式写法，从位开始读
			static word LZWreadWord(const stream &res, longex &startBitPos, int bitCount);		//LZW式读法，从位开始读
		public:
