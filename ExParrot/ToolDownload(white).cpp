...
DefineThreadFunc(ToolDownload, LoadFileList, CString) {
	...
	if(server.isTCT) {
		//国服资源（音效）
		LSTobjectGF lo2;
		if(lo2.load2(CStrToStr(listFileName))) {
		...
		if(lo.load(CStrToStr(listFileName))) {
			if(!lo.getSoundPack(lf)) {
				lc->InsertItem(0, L"列表内未能找到SoundPack文件夹。");
...
DefineThreadFunc(ToolDownload, DownloadFile, PVOID) {
	...
	const SERVER_INFO &server = serverList[s];
	CString sourStr = server.url + L"SoundPacks/";
	...
	for(int i = 0; i < lco->GetItemCount(); i++) {
		CString sourStr = server.url + L"SoundPacks/" + lco->GetItemText(i, 0) + (server.isTCT ? L".tct" : L".spk");
...
void ToolDownload::OnBnClickedButtonOpen() {
	...
	CExParrotDlg *dlg = (CExParrotDlg*)GetParent();
	if(dlg->NPKsaveAlert) {
		ModalSaveWarning ms;
		ms.alertType = dlg->SNDsaveAlert ? ModalSaveWarning::MODIFIED_SND_NPK : ModalSaveWarning::MODIFIED_NPK;
		ms.DoModal();
		switch(ms.returnType) {
		case ModalSaveWarning::RETURN_CANCEL:
			return;
		case ModalSaveWarning::RETURN_NO_SAVE:
			dlg->SNDsaveAlert = FALSE;
...
void ToolDownload::loadRecord(const CString &serverName) {
	...
	record.fileName.clear();
	record.completed.clear();
	if(file.Open(profile.getDownloadPath(serverName) + L"record.lso", CFile::modeRead)) {
		while(true) {
			char len = 0;
			char buf[0xFF];
			CString rn, rl;
...
void ToolDownload::updateRecord(const CString &serverName) {
	CFile file;
	file.Open(profile.getDownloadPath(serverName) + L"record.lso", CFile::modeCreate | CFile::modeWrite);
	for(int i = 0; i < record.fileName.size(); i++) {
		char len = 0;
...
void ToolDownload::removeRecord(const CString &serverName) {
	DeleteFile(profile.getDownloadPath(serverName) + L"record.lso");
}
